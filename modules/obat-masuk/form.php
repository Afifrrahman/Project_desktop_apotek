          ...
          <form role="form" class="form-horizontal" action="modules/obat-masuk/proses.php?act=insert" method="POST" name="formObatMasuk">
            <div class="box-body">
              ...
              <div class="form-group">
                ...
                <div class="col-sm-5">
                  <select class="chosen-select" name="pemasok" data-placeholder="-- Pilih Pemasok --" autocomplete="off" required>
                    <option value=""></option>
                    <?php
                      $query = mysqli_query($mysqli, "SELECT kode_pemasok, nama_pemasok, no_hp FROM is_pemasok ORDER BY nama_pemasok ASC")
                                                            or die('Ada kesalahan pada query tampil obat: '.mysqli_error($mysqli));
                      while ($data = mysqli_fetch_assoc($query)) {
                        echo"<option value=\"$data[kode_pemasok]\"> $data[kode_pemasok] | $data[nama_pemasok] </option>";
                      }
                    ?>
                  </select>
                </div>
